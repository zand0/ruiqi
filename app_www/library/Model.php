...
class Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        if(is_null($tablePrefix)) {// 前缀为Null表示没有前缀
            ...
            $this->tablePrefix = Yaf\Registry::get('config')->database->db_prefix;
        }
    ...
    protected function _checkTableInfo() {
        ...
        if(empty($this->fields)) {
            // 如果数据表字段没有定义则自动获取 
            if(Yaf\Registry::get('config')->database->db_fields_cache) {
                $db   =  $this->dbName?:$config['db_name'];
    ...
    public function __call($method,$args) {
        if(in_array(strtolower($method),$this->methods,true)) {
            ...
            throw new Db\Exception(__CLASS__.':'.$method.'METHOD_NOT_EXIST');
            return;
        }
    }
    // 回调方法 初始化模型
        ...
        if(!empty($this->fields)) {
            ...
            foreach ($data as $key=>$val){
                if(!in_array($key,$fields,true)){
                    if(APP_DEBUG){
                        throw new Db\Exception('DATA_TYPE_INVALID'.':['.$key.'=>'.$val.']');
                    }                    
                    unset($data[$key]);
     ...
     * @param boolean $update 数据已存在是否更新
     * @param mixed $option 已存在数据更新字段 数组或字符串
     * @return mixed
     */
    public function uinsert($data, $update = false, $option = array()) {
        $sql = 'INSERT INTO '.$this->getTableName().'(`'.implode('`,`', array_keys($data)).'`) VALUES("'.implode('","', $data).'")';

        if($update == true) {
            $sql .= $this->duplicateUpdate($option);
        }
     ...
     * @return boolean
     */
    public function uinsertAll($data) {
        $sql = 'INSERT INTO ' . $this->getTableName();
        $keys = $str = '';
        if (!empty($data)) {
            foreach ($data as $key => $value) {
                if ($key == 0) {
                    $keys = '(`' . implode('`,`', array_keys($value)) . '`) VALUES ';
                }
                $str .= '("' . implode('","', $value) . '"),';
            }
            $sql .= $keys . rtrim($str, ',');
    ...
    *
    */
    public function duplicateUpdate($fields){
        //增加update
        $sql = " ON DUPLICATE KEY UPDATE ";
        $update_str = '';
        if(is_array($fields)){
            foreach($fields as $keys=>$field) {
                    $update_str .= "`" . $keys . "` = '".$field . "',";
            }
        ...
        if(isset($options['where']) && is_array($options['where']) && !empty($fields) && !isset($options['join'])) {
            ...
            foreach ($options['where'] as $key=>$val){
                ...
                if(in_array($key,$fields,true)){
                    ...
                    if(APP_DEBUG){
                        throw new Db\Exception('ERROR_QUERY_EXPRESS'.':['.$key.'=>'.$val.']');
                    } 
    ...
    protected function _read_data($data) {
        ...
        if(!empty($this->_map) && $config['read_data_map']) {
     ...
     */
     public function create($data='',$type='') {
        ...
        if(isset($fields)) {
            ...
            // 判断令牌验证字段
            $config = Yaf\Registry::get('config'); 
            if($config['token_on'])   $fields[] = $config['token_name'];
            foreach ($data as $key=>$val){
    ...
    public function autoCheckToken($data) {
        ...
        if(C('TOKEN_ON')){
            $config = Yaf\Registry::get('config'); 
            $name   = $config['token_name'] ? $config['token_name']: '__hash__';
    ...
    public function db($linkNum='',$config='',$force=false) {
        ...
        if(!isset($_db[$linkNum]) || $force ) {
            // 创建一个新的实例
            $appConfig = Yaf\Registry::get('config');
    ...
    public function data($data=''){
        ...
        if(is_object($data)){
            ...
            throw new Db\Exception('DATA_TYPE_INVALID');
        }
        $this->data = $data;
        return $this;
    }
