...
class Mysql extends Db{
	...
	// 数据库配置临时变量(读写分离验证通过以后可以去掉)
	protected $tmp_config = null;
    ...
    public function __construct($config=''){
        if ( !extension_loaded('mysql') ) {
            throw new Db\Exception('NOT_SUPPERT'.':mysql'); 
        }
        if(!empty($config)) {
            $this->config   =   $config;
    ...
    public function connect($config='',$linkNum=0,$force=false) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            if ( !$this->linkID[$linkNum] || (!empty($config['database']) && !mysql_select_db($config['database'], $this->linkID[$linkNum])) ) {
                throw new Db\Exception(mysql_error());  
            }
            ...
            if(1 != $dbConfig['db_deploy_type']) unset($this->config);
        }
    ...
    public function query($str) {
        ...
        if(stripos($str,'insert') !== false || stripos($str,'update') !== false || stripos($str,'delete') !== false){
    ...
    protected function dbLog(){
    	$log_str = 'm.solarbao.com'.json_encode($this->tmp_config)."\n".$this->queryStr."\n";
    	//error_log($log_str, 3, "/tmp/db_log.log");
    }
