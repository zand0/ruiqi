...
class Pdo extends Db{
    ...
    public function connect($config='',$linkNum=0) {
        if ( !isset($this->linkID[$linkNum]) ) {
            ...
            try{
                $this->linkID[$linkNum] = new \PDO( $config['dsn'], $config['username'], $config['password'],$config['params']?$config['params']:array());
            ...
            if(in_array($this->dbType,array('MSSQL','ORACLE','IBASE','OCI'))) {
                ...
                throw new Db\Exception('由于目前PDO暂时不能完美支持'.$this->dbType.' 请使用官方的'.$this->dbType.'驱动');
            }
            $this->linkID[$linkNum]->exec('SET NAMES '.$config['charset']);
            // 标记连接成功
            $this->connected    =   true;
            // 注销数据库连接配置信息
            if(1 != \Yaf\Registry::get('config')->database->db_deploy_type) unset($this->config);
        }
    ...
    public function query($str,$bind=array()) {
        ...
        if(false === $this->PDOStatement)
           throw new Db\Exception($this->error());
        // 参数绑定
    ...
    public function getFields($tableName) {
        $this->initConnect(true);
        if( Yaf\Registry::get('config')->database->db_describe_table_sql ) {
            // 定义特殊的字段查询SQL
            $sql   = str_replace('%table%',$tableName,\Yaf\Registry::get('config')->database->db_describe_table_sql);
        }else{
            switch($this->dbType) {
                case 'MSSQL':
                case 'SQLSRV':
    ...
    public function getTables($dbName='') {
        if(C('DB_FETCH_TABLES_SQL')) {
            ...
            case 'IBASE':
                // 暂时不支持
               throw new Db\Exception('NOT_SUPPORT_DB'.':IBASE');
                break;
            case 'SQLITE':
