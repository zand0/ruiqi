...
class Page {
    ...
    public function __construct($totalRows, $listRows = 20, $parameter = array()) {
        LibF::C('site.var_page') && $this->p = LibF::C('site.var_page'); //设置分页参数名称
        /* 基础设置 */
        $this->totalRows = $totalRows; //设置总记录数
        $this->listRows = $listRows;  //设置每页显示行数
        $para = $_POST ? $_POST : $_GET;
        unset($para['__hash__'], $para['button']);
        foreach ($para as $k => $v) {
            if ($v == null) {
                unset($para[$k]);
            }
        }
        $p = empty($_POST['p']) ? (empty($_GET[$this->p]) ? 1 : intval($_GET[$this->p])) : intval($_POST['p']);
        $this->parameter = empty($parameter) ? $para : $parameter;
        $this->nowPage = $p;
    ...
    public function show() {
        ...
        //$controlleName = Yaf\Application::app()->getDispatcher()->getRequest()->getControllerName();
        $actionName = Yaf\Application::app()->getDispatcher()->getRequest()->getActionName();
        /* 生成URL */
        $this->parameter[$this->p] = '[PAGE]';
        $this->url = LibF::U($actionName, $this->parameter);
        /* 计算分页信息 */
