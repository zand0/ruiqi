...
class Log {
    ...
    static public function init($config=array()){
        ...
        $class  =   strpos($type,'\\')? $type: 'Log\\Driver\\'. ucwords(strtolower($type));           
        unset($config['type']);
    ...
    static function record($message,$level=self::ERR,$record=false) {
        if($record || false !== strpos(LibF::C('LOG_LEVEL'),$level)) {
    ...
    static function save($type='',$destination='') {
        ...
        if(empty($destination))
            $destination = LibF::C('LOG_PATH').date('y_m_d').'.log';
        if(!self::$storage){
            $type = $type?:LibF::C('LOG_TYPE');
            $class  =   'Log\\Driver\\'. ucwords($type);
