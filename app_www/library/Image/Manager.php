...
class Manager {
    ...
    public static function thumbnail($image, $cache_image, $size, $image_type = 'jpg') {
        ...
        if (!file_exists(IMAGE_CACHE_DIR . $cache_image)) {
            ...
            if (!\Image\Manager::checkImageMemoryLimit($image)) {
                return false;
            }
            ...
            if ($y < $size && $x <= $max_x) {
                ...
                \Image\Manager::resize($image, IMAGE_CACHE_DIR . $cache_image, $ratio_x, $size, $image_type);
            }
        }
    ...
    public static function cut($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $dst_x = 0, $dst_y = 0) {
        ...
        $src = array(
            'width' => $src_info[0],
            'height' => $src_info[1],
            'ressource' => \Image\Manager::create($src_info[2], $src_file),
        );
        ...
        $dest['ressource'] = \Image\Manager::createWhiteImage($dest['width'], $dest['height']);

        $white = imagecolorallocate($dest['ressource'], 255, 255, 255);
        imagecopyresampled($dest['ressource'], $src['ressource'], 0, 0, $dest['x'], $dest['y'], $dest['width'], $dest['height'], $dest['width'], $dest['height']);
        imagecolortransparent($dest['ressource'], $white);
        $return = \Image\Manager::write($file_type, $dest['ressource'], $dst_file);
    ...
    public static function resize($src_file, $dst_file, $dst_width = null, $dst_height = null, $file_type = 'jpg', $force_type = false) {
        ...
        $src_image = \Image\Manager::create($type, $src_file);
        ...
        if (!\Image\Manager::checkImageMemoryLimit($src_file))
            return false;
        ...
        return (\Image\Manager::write($file_type, $dest_image, $dst_file));
    }
    ...
    public static function validateUpload($file, $max_file_size = 0) {
        ...
        if (!\Image\Manager::isRealImage($file['tmp_name'], $file['type']) || !\Image\Manager::isCorrectImageFileExt($file['name']))
