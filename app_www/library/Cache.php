...
abstract class Cache {
    ...
    public static function getInstance() {
        if (!self::$instance) {
            $caching_system = \Yaf\Registry::get('config')->cache->caching_system;
            if (!empty($caching_system) && file_exists(dirname(__FILE__) . '/' . $caching_system . '.php')) {
                \Yaf\Loader::import(dirname(__FILE__) . '/' . $caching_system . '.php');
                self::$instance = new $caching_system();
                self::$instance->blacklist = \Yaf\Registry::get('cache_exclude_table');
