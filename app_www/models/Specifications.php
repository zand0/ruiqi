 ...
 * @author wjy
 */
class SpecificationsModel extends \Com\Model\Base {
    ...
    public function delData($id) {
        ...
        $where = array('id' => $id);
        $status = LibF::M('products_specifications')->where($where)->delete();
        if (!$status) {
            return $this->logicReturn('0206', '添加失败!');
        }
        return $this->logicReturn(200, 'ok', $status);
    ...
    public function getlist($param) {
        ...
        $productsModel = LibF::M('products_specifications');
    ...
    public function listData($param = array()) {
        $data = LibF::M('products_specifications')->where(array('status' => 0))->order('id DESC')->select();
        return $data;
    }
