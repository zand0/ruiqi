 ...
 * @author wjy
 */
class CylinderdepositModel extends \Com\Model\Base {
    ...
    public function add($data, $id) {
        ...
        if ($id) {
            $where = array('id' => $id);
            $status = LibF::M('cylinder_deposit')->where($where)->save($data);
        } else {
    ...
    public function delData($id) {
        ...
        $where = array('id' => $id);
        $status = LibF::M('cylinder_deposit')->where($where)->delete();
        if (!$status) {
            return $this->logicReturn('0206', '添加失败!');
        }
        return $this->logicReturn(200, 'ok', $status);
    ...
    public function getlist($params) {
        ...
        $cylinderModel = LibF::M('cylinder_deposit');
        ...
        $count = $cylinderModel->where($where)->count();
        $page = new Page($count, $pageSize);
        if ($offset > $count) {
            $data = array('count' => $count, 'list' => array(), 'filter' => $params, 'regionMap' => array());
            return $this->logicReturn(200, 'ok', $data);
        }
        $rows = $cylinderModel->where($where)->limit($offset, $pageSize)->order('id DESC')->select();
        $data = array('count' => $count, 'list' => $rows, 'filter' => $params, 'regionMap' => $regionMap, 'page' => $page);
        return $this->logicReturn(200, 'ok', $data);
    }

    public function listData($param = array(),$type = FALSE) {
        $returnData = array();
        
        $data = LibF::M('cylinder_deposit')->where(array('status' => 0))->order('id DESC')->select();
        if($type){
