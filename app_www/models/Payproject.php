 ...
 * @author wjy
 */
class PayprojectModel extends \Com\Model\Base {
    ...
    public function add($data, $id) {
        ...
        if ($id) {
            $where = array('id' => $id);
            $status = LibF::M('payproject')->where($where)->save($data);
        } else {
            $status = LibF::M('payproject')->add($data);
        }
        if (!$status) {
    ...
    public function delData($id) {
        ...
        $where = array('id' => $id);
        $status = LibF::M('payproject')->where($where)->delete();
        if (!$status) {
            return $this->logicReturn('0206', '添加失败!');
        }
        return $this->logicReturn(200, 'ok', $status);
    ...
    public function getlist($params) {
        ...
        $payprojectModel = LibF::M('payproject');
