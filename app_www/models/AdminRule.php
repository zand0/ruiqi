 ...
 * @date 2016/03/24
 */
class AdminRuleModel extends Com\Model\Base {
    ...
    public function getList() {
        return $this->tableD->order("pid asc,list_sort ASC")->where('status=1')->select();
    }
    ...
    public function getRuleList($authData) {
        ...
        if (!empty($authData)) {
            foreach ($authData as $key => $value) {
                ...
                $r['text'] = $text;
                $r['list'] = "<a href='/authrule/edite?id=" . $r['id'] . "'>修改</a>|<a href='/authrule/del?id=" . $r['id'] . "' onclick='if(confirm(\"确定删除?\")==false)return false;'>删除</a>";
