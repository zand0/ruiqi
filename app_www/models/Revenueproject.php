 ...
 * @author wjy
 */
class RevenueprojectModel extends \Com\Model\Base {
    ...
    public function add($data, $id) {
        ...
        if ($id) {
            $where = array('id' => $id);
            $status = LibF::M('revenueproject')->where($where)->save($data);
        } else {
            $status = LibF::M('revenueproject')->add($data);
        }
        if (!$status) {
    ...
    public function delData($id) {
        ...
        $where = array('id' => $id);
        $status = LibF::M('revenueproject')->where($where)->delete();
        if (!$status) {
            return $this->logicReturn('0206', '添加失败!');
        }
        return $this->logicReturn(200, 'ok', $status);
    ...
    public function getlist($params) {
        ...
        $revenueprojectModel = LibF::M('revenueproject');
