 ...
 * @date 2016/03/24
 */
class ManufacturerModel extends \Com\Model\Base {
    ...
    public function add($data, $id) {
        ...
        if ($id) {
            $where = array('id' => $id);
            $status = LibF::M('manufacturer')->where($where)->save($data);
        } else {
            $status = LibF::M('manufacturer')->add($data);
        }
        if (!$status) {
    ...
    public function delData($id) {
        ...
        $where = array('id' => $id);
        $status = LibF::M('manufacturer')->where($where)->delete();
        if (!$status) {
            return $this->logicReturn('0206', '添加失败!');
        }
        return $this->logicReturn(200, 'ok', $status);
    ...
    public function getlist($params) {
        ...
        $manufacturerModel = LibF::M('manufacturer');
        ...
        $count = $manufacturerModel->where($where)->count();
        $page = new Page($count, $pageSize);
        if ($offset > $count) {
            $data = array('count' => $count, 'list' => array(), 'filter' => $params, 'regionMap' => array());
            return $this->logicReturn(200, 'ok', $data);
        }
        $rows = $manufacturerModel->where($where)->limit($offset, $pageSize)->order('id DESC')->select();
        $data = array('count' => $count, 'list' => $rows, 'filter' => $params, 'regionMap' => $regionMap, 'page' => $page);
        return $this->logicReturn(200, 'ok', $data);
    }

    public function listData($param = array()) {
        $data = LibF::M('manufacturer')->where(array('status' => 0))->order('id DESC')->select();
        return $data;
    }

    public function getManufacturerArray($id = '') {

        $model = LibF::M('manufacturer');
        if ($id) {
            return $model->where('id=' . $id)->find();
        } else {
            $manufacturerObject = array();
            $data = $model->where(array('status' => 0))->select();
            if (!empty($data)) {
                foreach ($data as $value) {
                    $manufacturerObject[$value['id']] = $value;
                }
            }
