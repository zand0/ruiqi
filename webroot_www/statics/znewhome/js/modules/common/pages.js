...
define(['jquery'],function($){
    ...
    * */
    ...
    function createFirstPage(pager){
        var str = '<a href="javascript:void(0);" data-p="1" class="first-page page-icon" title="跳转到第一页"></a>';
        return str;
    }
    //创建上一页按钮
    function createPrevBtn(curPage){
        var str = '<a href="javascript:void(0);" data-p="'+curPage+'" class="prevPage"></a>';
        return str;
    }
    //创建页码
    function createPageNum(curPage,totalPage,showPageCount){
        ...
        var leftStart = 1,
            rightEnd = totalPage;
        //case1:左右两边都可以显示完整
        // if(curPage - showPageCount >= 1 && curPage + showPageCount <= totalPage){
        //    leftStart = curPage - showPageCount;
        //    rightEnd = curPage + showPageCount;
        // }
        ////case2:左边可以显示完整，右边不能显示完整
        // if(curPage - showPageCount >= 1 && curPage + showPageCount > totalPage){
        //    leftStart = curPage - showPageCount;
        //    rightEnd = totalPage;
        // }
        ////case3:左边不能显示完整，右边不能显示完整
        // if(curPage - showPageCount < 1 && curPage + showPageCount > totalPage){
        //    leftStart = 1;
        //    rightEnd = totalPage;
        // }
        ////case4:左边不能显示完整，右边可以显示完整
        if(curPage - showPageCount < 1 && curPage + showPageCount <= totalPage){
           leftStart = 1;
           rightEnd = curPage + showPageCount;
        ...
        for(var i=leftStart;i<=rightEnd;i++){
            if( i==curPage ){
                str+='<a href="javascript:void(0);" data-p="'+i+'" class="page-num on">'+i+'</a>';
            }else{
                str+='<a href="javascript:void(0);" data-p="'+i+'" class="page-num">'+i+'</a>';
            }
        }
    ...
    function createNextBtn(curPage){
        var str = '<a href="javascript:void(0);" data-p="'+curPage+'" class="nextPage"></a>';
        return str;
    }
    //创建尾页按钮
    function createLastPage(totalPage){
        var str = '<a href="javascript:void(0);" data-p="'+totalPage+'" class="last-page page-icon" title="跳转到最后一页"></a>';
        return str;
    }
    //创建Go按钮
    function createGoBtn(curPage){
        var str = '<span class="goPageNum">跳转到<input type="text" class="goPageInput" value="'+curPage+'" class="text-small"/>页</span>\
                    <a href="javascript:void(0);" class="goPageBtn">确定</a>';
        return str;
    }
    return PageNav;
});