...
Location.prototype.fillOption	= function(el_id , loc_id , selectedVal) {
	var el	= $('#'+el_id);
...
/*
* 容器选择器id：  el_province(省),el_city(市),el_town(区)
* 地区值 province(省) , city（市） , town（区）
* */
Location.prototype.setDefaults	= function(el_province,el_city,el_town,province , city , town) {
	var el_p = $('#'+el_province);
	var el_c = $('#'+el_city);
	var el_t = $('#'+el_town);
	var p_id,c_id,t_id;
	if(province){
		//生成省
		var json = this.find('0');
		$.each(json , function(k , v) {
			var option;
			if(v == province ){
				option	= '<option value="'+k+'" selected="selected">'+v+'</option>';
