        ...
        funcs.dialogFn = function () {
            $('.addCaigou').on('click', function () {
                ...
                $.ajax({
					...
					<div class="rqLine clearfix">\
						<div class="rqOption fl">\
							...
							<div class="selectBox fl">\
								<div class="mySelect">\
									<select name="" id="gas_list" class="selectmenu"  style="width:110px;">' + gasHtml + '</select>\
								</div>\
							</div>\
						</div>\
						<div class="rqOption fl">\
							<label class="fl">采购重量：</label>\
                ...
                $(dlg.node).find('#addplan').off('click').on('click', function () {
                    var gas_id = $("#gas_list option:selected").val();
                    var gas_name = $("#gas_list option:selected").text();
                    var supplier_id = $("#supplier_list option:selected").val();
                    var supplier_name = $("#supplier_list option:selected").text();
                    var num = $("#num").val();
                    var price = $("#price").val();
                    var blist = gas_id + '|' + supplier_id + '|' + num + '|' + price;
                    var fhtml = '<tr>\
                                        <td>' + gas_name + '</td>\
                                        <td>' + num + '</td>\
                                        <td>' + price + '</td>\
