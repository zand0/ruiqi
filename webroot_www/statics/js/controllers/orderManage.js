require(['/statics/js/config.js'], function () {
    require(['jquery', 'utils', 'modules/common/pages', 'vendors/jqueryUI/selectmenu'], function ($, utils, PageNav, selectmenu) {
        ...
        funcs.paifaFn = function () {
            ...
            //导出订单数据
            $(".deploy_order").on('click', function() {
                var paramlist = $(".paramlist").val();
                
                if (confirm('确认导出订单数据？')) {
                    var url = "/order/importorder";
                    if (paramlist != '') {
                        var paramlist = paramlist.replace(/(^\s*)|(\s*$)/g, "");
                        url += "?" + paramlist;
                    }
                    window.open(url);
                }
            ...
            $(".import_order").on('click', function() {
                ...
                if (paramData != '') {
                    var url = "/order/importorder?import_type=order&" + paramData;
                } else {
                    var url = "/order/importorder?import_type=order";
                }
            ...
            $(document).on('click', '.delorderBtn', function () {
                if(confirm('确认关闭此订单？')){
                    ...
                    $.ajax({
                        ...
                        async: false,
                        url: "/order/delorder",
                        data: {'ordersn':ordersn,'ordershop':ordershop},
                        success: function (result) {
                            var preview = eval("(" + result + ")");
                            if (preview.status >= 1) {
                                //location.reload();
